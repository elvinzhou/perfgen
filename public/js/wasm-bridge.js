...
// ── Pure-JS fallback — mirrors Rust logic exactly ─────────────────────────────

const WINDOW = 180;
const ALT_RANGE = 100, IAS_RANGE = 4, RPM_RANGE = 40, MAP_RANGE = 0.4, ROLL_MAX = 3;

const isaTemp = hp => 15 - 0.0019812 * hp;
const densityAlt = (hp, oat) => hp + 118.8 * (oat - isaTemp(hp));
const tasFromIas = (ias, hp, oat) => {
  const sigma = Math.pow(1 - 6.8755856e-6 * hp, 5.2558797) * (288.15 / (oat + 273.15));
  return ias / Math.sqrt(sigma);
};
const ambientP = hp => 29.92 * Math.pow(1 - 6.8755856e-6 * hp, 5.2558797);
const valRange = arr => Math.max(...arr) - Math.min(...arr);

function isSteady(win) {
  return valRange(win.map(r => r.pres_alt)) <= ALT_RANGE
    && valRange(win.map(r => r.ias)) <= IAS_RANGE
    && valRange(win.map(r => r.rpm)) <= RPM_RANGE
    && valRange(win.map(r => r.map_inhg)) <= MAP_RANGE
    && win.every(r => Math.abs(r.roll) <= ROLL_MAX);
}
...
function buildResult(rec) {
  ...
  const da = !isNaN(rec.da) ? rec.da : densityAlt(pa, rec.oat);
  const tas = !isNaN(rec.tas) ? rec.tas : tasFromIas(rec.ias, pa, rec.oat);
  const ap = ambientP(pa);
  const pwr = rec.pwr_pct > 0 ? rec.pwr_pct : Math.min((rec.map_inhg / ap) * 100, 100);
  const sr = rec.fuel_flow_gph > 0.1 ? rec.gnd_spd / rec.fuel_flow_gph : NaN;
  ...
  return {
    ...
    specific_range: isNaN(sr) ? NaN : r1(sr),
...
function jsFallback(csvText, _maxHp) {
  ...
  for (const line of lines.slice(headerIdx + 1)) {
    ...
    records.push({
      ...
      pwr_pct:g(cols,pwrCol), fqty_total:(g(cols,fq1Col)||0)+(g(cols,fq2Col)||0),
  ...
  if (records.length < WINDOW)
    return { steady_state_blocks:[], total_records:records.length, skipped_records:skipped, error:`Insufficient records: ${records.length}` };
